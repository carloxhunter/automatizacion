...
update_models_script = "bash /scripts/get_models.sh"
models_dict={
...
def returnParse():
    parser = argparse.ArgumentParser(description='Prepare configuration file.')
    parser.add_argument('--token', default='00000', type=str, help='unique token of client')
...
def update_model_sh(token, path=model_script_path, models=models_dict):
    bash = open(path, "a")
    user_info = getClientInfo(token)

    pgie = user_info['modelo_primario']
    cmd = "rclone sync cloud_modelos:modelsdigevo/"+models[pgie]+" /models/"+models[pgie]
    bash.write(cmd)
    bash.write("\n")

    sgies = user_info['modelo_secundario']
    for i in sgies:
...
if __name__ == "__main__":
    args = returnParse()
    config_info = parseConfigFile(args.token)
